...
// Phase 0 decision: 3 task_type nodes only (io/network/compute).
...
// D3 simulation node must have optional fx/fy for dragging
interface SimNode {
    id: string
    label: string
    task_type: string
    ...
    useEffect(() => {
        ...
        import('d3').then((d3) => {
            ...
            node.append('circle').attr('r', 36)
                .attr('fill', (d) => TASK_TYPE_COLORS[d.task_type] ?? '#3b82f6')
                .attr('fill-opacity', 0.1)
                .attr('stroke', (d) => TASK_TYPE_COLORS[d.task_type] ?? '#3b82f6')
